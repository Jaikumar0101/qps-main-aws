...
class PostCollectionListPage extends Component
{
    use WithPagination;
    ...
    public function render()
    {
        ...
        {
            $data = PostCollection::where(function ($q){
        ...
        {
            $data = PostCollection::orderBy($this->filter['sortBy'],$this->filter['orderBy'])
                ->paginate($this->filter['perPage']);
        }
        return view('livewire.admin.blog.collection.post-collection-list-page',compact('data'))
    ...
    public function exportData():void
    {
        ...
        PostCollection::orderBy($this->filter['sortBy'],$this->filter['orderBy'])
            ->chunk(10000, function ($history) use ($header_style) {
                $filename = time()."export_post_collection.xlsx";
