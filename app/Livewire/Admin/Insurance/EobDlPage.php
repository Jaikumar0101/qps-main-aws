...
class EobDlPage extends Component
{
    use WithPagination;
    ...
    public function render()
    {
        ...
        {
            $data = InsuranceEobDl::where(function ($q){
        ...
        {
            $data = InsuranceEobDl::orderBy($this->filter['sortBy'],$this->filter['orderBy'])
                ->paginate($this->filter['perPage']);
        }
        return view('livewire.admin.insurance.eob-dl-page',compact('data'));
    }
    public function save()
    {
        if (\Arr::has($this->request,'id'))
        {
            ...
            try
            {
                $check = InsuranceEobDl::find($this->request['id']);
        ...
        else
        {
            ...
            try
            {
                InsuranceEobDl::create($this->request);
                $this->NewRequest();
    ...
    public function exportData():void
    {
        ...
        InsuranceEobDl::orderBy($this->filter['sortBy'],$this->filter['orderBy'])
