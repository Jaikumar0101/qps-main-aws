...
class FollowUpPage extends Component
{
    ...
    public function render()
    {
        ...
        {
            $data = InsuranceFollowUp::where(function ($q){
        ...
        {
            $data = InsuranceFollowUp::orderBy($this->filter['sortBy'],$this->filter['orderBy'])
                ->paginate($this->filter['perPage']);
        }
        return view('livewire.admin.insurance.follow-up-page',compact('data'));
    }
    ...
    public function save()
    {
        if (\Arr::has($this->request,'id'))
        {
            ...
            try
            {
                $check = InsuranceFollowUp::find($this->request['id']);
        ...
        else
        {
            ...
            try
            {
                InsuranceFollowUp::create($this->request);
                $this->NewRequest();
    ...
    public function OpenAddEditModal($id = null)
    {
        if (checkData($id))
        {
            $check = InsuranceFollowUp::find($id);
            if ($check)
            {
    ...
    public function exportData():void
    {
        ...
        InsuranceFollowUp::orderBy($this->filter['sortBy'],$this->filter['orderBy'])
