...

{
    ...
    public function render()
    {
        ...
        {
            $data = InsuranceClaimTask::where(function ($q){
        ...
        {
            $data = InsuranceClaimTask::orderBy($this->filter['sortBy'],$this->filter['orderBy'])
                ->paginate($this->filter['perPage']);
        }
        return view('livewire.admin.insurance.task-list-page',compact('data'));
    }
    ...
    public function save()
    {
        if (\Arr::has($this->request,'id'))
        {
            ...
            try
            {
                $check = InsuranceClaimTask::find($this->request['id']);
        ...
        else
        {
            ...
            try
            {
                InsuranceClaimTask::create($this->request);
                $this->NewRequest();
    ...
    public function OpenAddEditModal($id = null)
    {
        if (checkData($id))
        {
            $check = InsuranceClaimTask::find($id);
            if ($check)
            {
    ...
    public function exportData():void
    {
        ...
        InsuranceClaimTask::orderBy($this->filter['sortBy'],$this->filter['orderBy'])
