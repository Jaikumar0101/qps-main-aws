...
use App\Models\InsuranceTflExcluded;
use Livewire\Component;
use Livewire\WithPagination;
use Rap2hpoutre\FastExcel\FastExcel;

class TlfExcludedPage extends Component
{
    use WithPagination;
    ...
    public function render()
    {
        ...
        {
            $data = InsuranceTflExcluded::where(function ($q){
        ...
        {
            $data = InsuranceTflExcluded::orderBy($this->filter['sortBy'],$this->filter['orderBy'])
                ->paginate($this->filter['perPage']);
        }
        return view('livewire.admin.insurance.tlf-excluded-page',compact('data'));
    }
    ...
    public function save()
    {
        if (\Arr::has($this->request,'id'))
        {
            ...
            try
            {
                $check = InsuranceTflExcluded::find($this->request['id']);
        ...
        else
        {
            ...
            try
            {
                InsuranceTflExcluded::create($this->request);
                $this->NewRequest();
    ...
    public function OpenAddEditModal($id = null)
    {
        if (checkData($id))
        {
            $check = InsuranceTflExcluded::find($id);
            if ($check)
            {
    ...
    public function exportData():void
    {
        ...
        InsuranceTflExcluded::orderBy($this->filter['sortBy'],$this->filter['orderBy'])
